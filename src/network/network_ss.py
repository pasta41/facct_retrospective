...
# python network_ss.py ../data/combined.json ../network_output ../data/ss_levels.json louvain_dir ../DirectedLouvain/ ../data/ALLIDS --skip_extraction

class GraphGenerator:
	"""
	A class encapsulating the generation of a citation network of a subset of the articles from the Aminer/S2ORC dataset
	...
	def extract_records(self):
		...
		for item in ijson.items(self.sourceFile, "item", use_float = True):
			...
			try:
				...
				pass
		extractedFile = os.path.join(os.path.dirname(self.sourceFilePath), "extracted_records_s2orc.json")
		with open(extractedFile, 'w') as f_write:		
			json.dump(self.node_records, f_write, indent = 0)

	def edge_list(self):
		edge_list = []
		edgelistFile = os.path.join(os.path.dirname(self.sourceFilePath), "ss_edgelist.dat")
		with open(edgelistFile, 'w') as f_write:	
			for node_id, node_record in self.node_records.items():
				if node_record["outbound_citations"]:
					references = node_record["outbound_citations"]
					for reference_id in references:
						if reference_id in self.node_records:
...
def write_components_to_csv(filePath, labelview_dict, g, node_levels=None):
	with open(filePath, 'w', newline='') as csvfile:
		csv_writer = csv.writer(csvfile)
		csv_writer.writerow(["component_label", "node_id", "article_title", "in_degree", "level", "incoming_0", "incoming_-1", "incoming_1"])
		for component_label in sorted(labelview_dict.keys()): 
			...
			for node in component:
				...
				in_levels = Counter()
				for u, v in in_edges:
					in_level = node_levels[u]
					in_levels[in_level] += 1
				csv_writer.writerow([component_label, node, g.nodes[node]['article_title'], g.in_degree[node], node_level, in_levels[0], in_levels[-1], in_levels[1]])
			csv_writer.writerow(['.............', '............'])

def plot_graph(G):
	node_mapping = {0: "TBD", 1: "Fairness in ML", 2: "Explainability", 3: "Bias in Word Embeddings", 4: "Bias in Image Recognition", 5: "Privacy", 6:"Bias in Search", 7:"RecSys.", 8: "Fairness and Ethnicity", 9:"TBD", 10:"Social Media / Mental Health", 11: "TBD", 12: "Algorithmic Econ.", 13: "TBD", 14: "GDPR"}
	...
	nx.draw_networkx_edges(G, pos, 
						   ...
						   edgelist = widths.keys(),
						   width=1,
						   edge_color=[np.log(adjusted_width) for adjusted_width in list(widths.values())],
...
def main():
	...
	write_components_to_csv(filePath, partition_main.labelview, g, levels)
	...
	if algorithm in ["louvain", "louvain_dir"]:
		for component_label, component in partition_main.labelview.items():
			...
			write_components_to_csv(filePath, partition_sub.labelview, subgraph_g, levels)
